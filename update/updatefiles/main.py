...
def updatePaymentStatus():
    os.system("python updatepayment.py")
    return
...
def backup_generated(billid2backup):
    backuppath = r'C:/Users/PC/Documents/fbbackup/'+billid2backup+'.zip'
    os.makedirs(os.path.dirname(backuppath), exist_ok=True)
    archived = shutil.make_archive(r'C:/Users/PC/Documents/fbbackup/'+billid2backup, 'zip', './generatedbills/'+billid2backup)
    return
...
def paymentdata_recordexists(billno_text, billedamnt):
    print("checking payment record")
    paymentrecord = conn.cursor().execute("SELECT * FROM paymentdata WHERE billno = ?", (billno_text, )).fetchall()
    print("paymentrecord: ", paymentrecord)
    if len(paymentrecord) == 0:
        newjsontext = f"[[], {billedamnt}, {billedamnt}]"
        conn.execute("INSERT INTO paymentdata VALUES (NULL, ?, ?, ?)", (billno_text, newjsontext, str(int(time.time())), )) # [ 
            #                                                                                           [ 
            #                                                                                               [modeofpayment, date, amount, comment], 
            #                                                                                               [modeofpayment, date, amount, comment], 
            #                                                                                               [modeofpayment, date, amount, comment] 
            #                                                                                           ], 
            #                                                                                           dueamount, 
...
def newBill(current_billno,
    ...
    try:
        ...
        conn.rollback()
        conn.commit()
        newrecordadd_successful = False
    if newrecordadd_successful:
        paymentdata_recordexists(current_billno, sumtotal)
    return
